...
this.animateVisible = function(options) {
	var opt = options || {};
	var $window = $(window);
	...
	if((!bgSwitcher || !bgSwitcher.length) && opt.backgroundImage) {
		bgSwitcher = $("<div></div>");
		bgSwitcher.attr("data-background-image", "url(\'" + opt.backgroundImage + "\'");
	}

	if(bgSwitcher && bgSwitcher.length) {
		...
		newImage.fadeIn("slow", function() {
			...
			newImage.addClass("remove-old-bg ken-burns");
		});
	}
};

this.animae = function(options) {
	let opt = options || {};
